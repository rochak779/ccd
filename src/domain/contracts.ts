...
export const locatorSchema = z.discriminatedUnion("kind", [
  ...
  z.object({ kind: z.literal("csv_cell"), row: z.number().int().min(2), column: z.string().min(1) }),
  z.object({ kind: z.literal("text_lines"), startLine: z.number().int().positive(), endLine: z.number().int().positive() }).refine(({ startLine, endLine }) => endLine >= startLine, "Text line range must be ordered"),
...
export const messageSchema = z.object({ id, dealId: id, source: z.enum(["SEED", "GMAIL"]), providerMessageId: id, internetMessageId: z.string().min(1), threadId: id, inReplyTo: z.string().nullable(), references: z.array(z.string().min(1)).default([]), from: email, to: z.array(email), cc: z.array(email), subject: z.string(), sentAt: utc, receivedAt: utc.optional(), bodyText: z.string(), bodyHash: sha256, attachmentIds: z.array(id), classification: z.enum(["DILIGENCE_REQUEST", "DILIGENCE_RESPONSE", "SCHEDULING_NOISE"]) });
